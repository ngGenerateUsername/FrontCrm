...
import { Prop, contactsPerEntreprise, contactsPerFournisseur } from "state/user/Role_Slice";
import { fetchSingleUserFournisseur } from "state/Fou_slice/Fou_slice";
function Overview() {
  ...
  useEffect(() => {
    dispatch(fetchSingleUserFournisseur(params.get("id")) as any)
      .unwrap()
      .then((res: any) => {
  ...
  const { status: statusEntreprise, record: recordEntreprise } = useSelector(
    (state: any) => state.fetchSingleUserFournisseurExport
  );
  console.log(recordEntreprise, statusEntreprise);

  useEffect(() => {
    dispatch(contactsPerFournisseur(params.get("id")) as any)
      .unwrap()
      .then((res: any) => {
        console.log(res.idUser);
      })
  ...
  const { status: statusContacts, record: recordContacts } = useSelector(
    (state: any) => state.contactsPerFournisseurExport
  );
    ...
    <Box pt={{ base: "130px", xl: "80px", md: "20px" }}>
      <Grid
        ...
        <Banner
          ...
          name={recordEntreprise.nomFournisseur}
